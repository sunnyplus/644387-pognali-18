...
document.addEventListener("DOMContentLoaded", function () {
  mainnavContent.classList.add("main-nav__content--closed"); // close menu
  if(document.querySelector(".map__iframe--nojs")) { // показываем интерактивную карту
    document.querySelector(".map__iframe--nojs").classList.remove("map__iframe--nojs");
  }
  if(document.querySelector(".map__nojs")){
    document.querySelector(".map__nojs").classList.add("map__nojs--closed");
  }
  if(document.querySelector(".logo__wrapper--nojs-blue")) {
    document.querySelector(".logo__wrapper--nojs-blue").classList.remove("logo__wrapper--nojs-blue");
  }
  if(document.querySelector(".logo__wrapper--nojs-white")) {
    document.querySelector(".logo__wrapper--nojs-white").classList.remove("logo__wrapper--nojs-white");
  }
  if(logoWrapper) {
  ...
  mainnavContent.classList.add("main-nav__content--closed");
  pageHeader.classList.add("page-header--closed"); // меняем бг синий/белый------------
});
...
mainnavBtn.addEventListener('click', function(evt) {
  evt.preventDefault();
  mainnavContent.classList.toggle("main-nav__content--closed"); // закрываем/открываем меню--------------
});
...
if(yellowFormBtn) { // закрываем/открываем пункты жёлтого меню по клику
  for(var a=0; a<yellowFormBtn.length; a++) {
    ...
    yellowFormBtn[a].addEventListener('keydown', function(evt) {
      // evt.preventDefault();
      if(evt.keyCode == 13) {
        this.parentElement.classList.toggle("companion-select__field--closed");
      }
